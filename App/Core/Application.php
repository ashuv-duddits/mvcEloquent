...
class Application
{
    ...
    public function run()
    {
        ...
        // создаем контроллер
        $currentName = 'App\controllers\\' . $this->controllerName;
        $controller = new $currentName();
        if (!($controller instanceof BaseController)) {
            throw new \Exception('Controller ' . $this->controllerName . ' not implement abstract controller');
        }

        // проверяем существование метода
        if (!method_exists('App\controllers\\' . $this->controllerName, $this->actionName)) {
            throw new \Exception(
